...
// (partially old) code that handles the LaserTag specific WebSocketserver and format

public partial class LaserTag(BaseMapLayer tileMapLayer) : Node
{
	...
	public override void _Ready()
	{
		...
		client.OnMessage += (string message) =>
		{
			...
			DrawGame(model);
			// FIXME: since we don't have access to the config anymore,
			//        there needs to be another way to tell, wether the game has ended
			//        - probably best if we have access to the config afterall then we
			//          know where the project root is and can in the future maybe do other
			//          things as well
			//        - ! in this impl we have access to it
	...
	private void ShowScores()
	{
		...
		var endScoreLabel = parent.GetNode<RichTextLabel>("%EndScoreLabel");
		...
		foreach (var jsonAgent in jsonAgents)
		{
			...
			if (agent.Alive && !jsonAgent.Alive)
			{
				if (jsonAgent.TaggerId == "00000000-0000-0000-0000-000000000000")
					parent.GetNode<KillFeed>("%KillFeed").AddKill(agent);
				else
				{
					var killer = existingAgents[jsonAgent.TaggerId];
					parent.GetNode<KillFeed>("%KillFeed").AddKill(killer, agent);
				}
			}
